...
const HideInterfaceButton: React.FC<HideInterfaceButtonProps> = ({ isHidden, onPress, onRelease }) => {
    ...
    return (
        <button
            ...
            ${isHidden ? 'bg-white/20 text-white ring-1 ring-white/30' : 'bg-[rgba(25,25,28,0.92)] hover:bg-[rgba(35,35,38,0.95)]'}
      `}
        ...
        >
            <svg
                ...
                viewBox="0 0 24 24"
                className={`w-5 h-5 ${isHidden ? 'eye-icon--hidden' : 'eye-icon--visible'} text-current transition-colors duration-150`}
                aria-hidden="true"
            >
                {isHidden ? (
                    <path
